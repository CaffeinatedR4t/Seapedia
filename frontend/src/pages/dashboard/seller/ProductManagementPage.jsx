...
import { Store, Package, Image as ImageIcon, Edit2, Trash2, Plus, X } from 'lucide-react'
      ...
      <div className="flex min-h-[calc(100vh-64px)] bg-paper-50">
        ...
        <main className="flex-1 p-8">
          <div className="card p-8 text-center max-w-lg mx-auto mt-12">
            <span className="text-4xl mb-4 flex justify-center text-coral-600"><Store size={48} /></span>
            <h2 className="text-xl font-bold text-ink-800 mb-2">Toko Belum Dibuat</h2>
            <p className="text-ink-500 mb-6">Kamu harus membuat profil toko terlebih dahulu sebelum bisa menambahkan produk.</p>
            <a href="/seller/store" className="btn-md btn-primary">Buat Profil Toko</a>
          </div>
        </main>
      </div>
    )
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-ink-900 mb-1 flex items-center gap-2">
              <Package className="text-coral-600" /> Manajemen Produk
            </h1>
            <p className="text-ink-500">Kelola katalog produk untuk tokomu</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {products.map(p => (
            <div key={p.id} className="card overflow-hidden flex flex-col">
              <div className="h-48 bg-paper-200 relative">
                {p.image_url ? (
                  <img src={p.image_url} alt={p.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-paper-100 flex items-center justify-center text-ink-400">
                    <ImageIcon size={48} />
                  </div>
                )}
              </div>
              <div className="p-4 flex-1 flex flex-col">
                <h3 className="font-bold text-ink-800 line-clamp-1" title={p.name}>{p.name}</h3>
                <p className="text-sm text-ink-500 line-clamp-2 mt-1 flex-1">{p.description || 'Tidak ada deskripsi'}</p>
                <div className="mt-4 flex justify-between items-end">
                  <div>
                    <p className="text-xs text-ink-500 mb-1">Harga & Stok</p>
                    <p className="font-bold text-coral-600">Rp {p.price.toLocaleString('id-ID')}</p>
                    <p className="text-xs text-ink-500 mt-1">Stok: {p.stock}</p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => handleOpenForm(p)} className="p-2 text-ink-600 bg-paper-100 rounded-lg hover:bg-paper-200 transition-colors">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(p.id)} className="p-2 text-error bg-error/10 rounded-lg hover:bg-error/20 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
          ...
          <div className="fixed inset-0 bg-ink-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="card w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto animate-slide-up">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-ink-800">{editingId ? 'Edit Produk' : 'Tambah Produk Baru'}</h2>
                <button onClick={() => setIsFormOpen(false)} className="text-ink-400 hover:text-ink-600 text-xl">
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="pt-4 flex justify-end gap-3">
                  <button type="button" onClick={() => setIsFormOpen(false)} className="btn-md btn-ghost text-ink-600">Batal</button>
